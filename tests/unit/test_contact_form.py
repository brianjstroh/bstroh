            ...
            "main": [
                {
                    ...
                    "data": {
                        "left_slot": [
                            {"type": "text-heading", "data": {"heading": "Info"}}
                        ],
                        "right_slot": [
                            {
                                "type": "contact-form",
...
class TestLambdaHandler:
    ...
    def test_valid_submission_returns_success(
        ...
        with patch("form_submission.index.ssm", mock_ssm), patch(
            "form_submission.index.ses", mock_ses
        ):
            from form_submission.index import handler

            event = {
                "body": '{"name": "John", "email": "john@test.com", "message": "Hello", "form_id": "contact"}',
    ...
    def test_invalid_email_returns_error(
        ...
        with patch("form_submission.index.ssm", mock_ssm), patch(
            ...
            event = {
                "body": '{"name": "John", "email": "not-an-email", "message": "Hello", "form_id": "contact"}',
                "headers": {"origin": "https://example.com"},
            }
...
class TestDynamicFormFields:
    ...
    def test_dynamic_fields_too_many_fields_rejected(
        ...
        with patch("form_submission.index.ssm", mock_ssm), patch(
            ...
            fields = [
                {"label": f"Field{i}", "type": "text", "value": f"val{i}"}
                for i in range(51)
            ]
            import json
