    ...
    for field in fields:
        ...
        if field_type == "email":
            ...
            if value and not EMAIL_REGEX.match(value):
                return response(
                    400, {"success": False, "error": f"Invalid email in {label}"}
                )
            if value:
                sender_email = value
...
def send_confirmation_email(
    ...
    try:
        ...
        html_body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: {bg_color}; color: {text_color};">
            <div style="border-left: 4px solid {primary_color}; padding-left: 15px; margin-bottom: 20px;">
                <p style="margin: 0;">{html.escape(message)}</p>
            </div>
            <hr style="border: none; border-top: 1px solid {border_color}; margin: 20px 0;">
            <p style="color: {text_muted}; font-size: 14px;"><strong>Your submission:</strong></p>
            {build_html_email(domain, "", fields, include_header=False, theme=theme)}
        </body>
        </html>
        """
...
def build_html_email(
    ...
    for field in fields:
        ...
        if field_type == "address" and isinstance(value, dict):
            ...
            city_state_zip = (
                f"{value.get('city', '')}, {value.get('state', '')} "
                f"{value.get('zip', '')}"
            )
            ...
            field_rows.append(
                f'<div style="margin-bottom: 15px;">'
                f'<strong style="color: {text_color};">{label}:</strong> '
                f'<a href="mailto:{esc_val}" style="color: {primary_color};">{esc_val}</a></div>'
            )
    ...
    if include_header:
        return f"""
        <html>
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: {bg_color};">
            <h2 style="color: {primary_color}; margin-bottom: 25px; padding-bottom: 15px; border-bottom: 2px solid {border_color};">New Form Submission</h2>
            {''.join(field_rows)}
            <hr style="border: none; border-top: 1px solid {border_color}; margin: 25px 0 15px 0;">
